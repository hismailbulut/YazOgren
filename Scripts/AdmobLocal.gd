...
func _on_AdMob_rewarded_video_closed():
	if _is_rewarded:
		get_parent().get_node("main_control/SplitManager/GamePanel/PauseMenu").on_rewarded()
	else:
		get_parent().get_node("main_control/SplitManager/GamePanel/PauseMenu").on_rewarded_closed()
